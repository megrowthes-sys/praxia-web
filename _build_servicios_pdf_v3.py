"""Build Praxia_Atelier_Servicios.pdf · v3.

Mejoras vs v2:
- Igual estética que el rediseño del hero web (claim + cursiva + aterrizaje).
- Cero solapamiento: usamos columnas fijas y medimos cada bloque antes de
  pintar. El número editorial gigante vive en una columna IZQUIERDA propia
  y el contenido textual en otra DERECHA. Nunca pisan.
- Identidad fiel: Fraunces serif + IBM Plex sans/mono, paleta papel/tinta/
  terracota, kickers mono uppercase, hairlines como decoración editorial.
- Pull-quote con comillas tipográficas grandes en color acento.
- Botones rectangulares estilo brutal con sombra sólida.
"""
...
def register_fonts():
    ...
    for name, path in fonts:
        if not path.exists():
            FONTS_OK = False
            print(f"WARN missing font {path}")
            continue
        try:
...
HEADER_HEIGHT  = 14 * mm   # space reserved for top brand band
FOOTER_HEIGHT  = 12 * mm   # space reserved for footer band

# Y where actual content starts (top) and ends (bottom)
...
# Drawing helpers
# ============================================================
def text_h(text, font, size, w, leading=None):
    """Compute height in points for wrapped text block."""
    if leading is None: leading = size * 1.42
    lines = simpleSplit(text, font, size, w)
    return len(lines) * leading, lines, leading


def draw_text(c, text, x, y, w, font, size, color, leading=None, max_lines=None):
    """Draw wrapped text from a given top-y baseline. Returns Y after the block."""
    _, lines, leading = text_h(text, font, size, w, leading)
...
def draw_button(c, label, url, x, y, w, h,
    ...
    """Pinta un botón rectangular con sombra opcional estilo brutal y registra link."""
    if font is None: font = F_SANS_MED
    if shadow:
        sc = shadow_color or INK
        c.setFillColor(sc)
        c.rect(x + 1.5*mm, y - 1.5*mm, w, h, stroke=0, fill=1)
    if fill is not None:
        c.setFillColor(fill); c.rect(x, y, w, h, stroke=0, fill=1)
    if border is not None:
        c.setStrokeColor(border); c.setLineWidth(0.7)
        c.rect(x, y, w, h, stroke=1, fill=0)
    c.setFillColor(text or white); c.setFont(font, font_size)
...
def header_band(c, page_num, total_pages):
    """Top band: brand left + page indicator right + hairline below."""
...
COPY_ES = {
    ...
    # Page 1 hero (mirrors the new website)
    "p1_h1_a": "Diseñamos ecosistemas digitales",
    "p1_h1_b": "listos para captar capital,",   # accent color
    "p1_h1_c": "multiplicar el ROI o, simplemente, saber qué hacer la próxima semana.",  # italic
    "p1_aterrizaje": ("Aterrizamos tu idea y la convertimos en un "
                       "negocio real, digitalizado y optimizado — con sistema "
                       "operativo, página web y todo lo que necesitas para empezar "
                       "a funcionar."),
    "p1_aterrizaje_highlight": "negocio real, digitalizado y optimizado",
    ...
    "idea_lead":   ("Tienes una intuición de negocio pero no sabes "
                     "por dónde empezar a estructurarla."),
    "idea_caption":"Empezamos por ahí — en un brainstorm te devolvemos opciones reales.",
    "comp_kicker": "EMPRESA CON NUEVA VÍA",
    "comp_lead":   ("Tu empresa funciona pero quieres abrir una nueva "
                     "vía sin canibalizar el core."),
    ...
    "p2_block2_body": ("Un ecosistema digital con dos caras. La web pública, "
                       "con cuatro puertas — invertir, vender, alquilar, buscar piso. "
                       "Y el back office propio, con vista panorámica de los diez "
                       "procesos del negocio: desde el catálogo hasta el cash flow "
                       "proyectado mes a mes."),
    ...
    "p2_others_2_d": ("Ecosistema emocional propio con IA en producción. Tres "
                       "líneas (B2C · marketplace · enterprise)."),

    # Page 3 · 4 fictional cases
    ...
    "p4_h1_sub": ("Cada cápsula tiene alcance, plazo y entregables fijos. Si no "
                   "encajan, no hay propuesta."),
    "capsules": [
...
COPY_EN.update({
    ...
    "p1_h1_a": "We design digital ecosystems",
    "p1_h1_b": "ready to raise capital,",
    "p1_h1_c": "multiply ROI or, simply, to know what to do next week.",
    "p1_aterrizaje": ("We ground your idea and turn it into a real, "
                       "digitalised and optimised business — with operating "
                       "system, website and everything you need to start running."),
    "p1_aterrizaje_highlight": "real, digitalised and optimised business",
    ...
    "promise_quote": ("Book thirty minutes and within 48 hours you receive "
                      "a free one-pager with three actionable moves specific "
                      "to your project."),
    ...
    "p2_block2_body": ("A digital ecosystem with two faces. The public website with "
                       "four entry doors — invest, sell, rent, search. And the "
                       "custom back office with a panoramic view of the ten business "
                       "processes: from the catalogue to monthly projected cash flow."),
...
# ============================================================
# Page 1 · Hero (espejo del rediseño web)
# ============================================================
def page1(c, t):
    header_band(c, 1, 4)
    ...
    # H1 line A · "Diseñamos ecosistemas digitales"
    c.setFillColor(INK); c.setFont(F_SERIF_BOLD, 38)
    c.drawString(MARGIN_X, y, t["p1_h1_a"])
    y -= 12 * mm
    # H1 line B · accent · "listos para captar capital,"
    c.setFillColor(ACCENT); c.setFont(F_SERIF_BOLD, 38)
    c.drawString(MARGIN_X, y, t["p1_h1_b"])
    y -= 11 * mm
    # H1 line C · italic ink-soft, smaller, may wrap
    c.setFillColor(INK_SOFT); c.setFont(F_SERIF, 22)
    lines_c = simpleSplit(t["p1_h1_c"], F_SERIF, 22, CONTENT_W - 5 * mm)
    for ln in lines_c[:2]:
        c.drawString(MARGIN_X, y, ln); y -= 9 * mm
    y -= 6 * mm
    ...
    # Aterrizaje block
    y = draw_text(c, t["p1_aterrizaje"], MARGIN_X, y,
                  CONTENT_W - 5 * mm, F_SERIF, 12.5, INK, leading=18)
    y -= 8 * mm

    # CTAs
    btn_h = 13 * mm
    bw1 = c.stringWidth(t["btn_primary"], F_SANS_MED, 11) + 16 * mm
    draw_button(c, t["btn_primary"], CAL_URL,
                MARGIN_X, y - btn_h, bw1, btn_h,
                fill=ACCENT, text=PAPER, font_size=11, shadow=True, shadow_color=INK)
    # Secondary as outline
    bw2 = c.stringWidth(t["btn_secondary"], F_SANS_MED, 10.5) + 14 * mm
    draw_button(c, t["btn_secondary"], f"{WEB_URL}/#casos",
                MARGIN_X + bw1 + 6 * mm, y - btn_h, bw2, btn_h,
                fill=None, text=INK, border=INK, font_size=10.5)
    # caption right
    cap_x = MARGIN_X + bw1 + 6 * mm + bw2 + 6 * mm
    c.setFont(F_MONO, 7); c.setFillColor(INK_FAINT)
    c.drawString(cap_x, y - btn_h + (btn_h / 2) - 2, "· " + t["btn_caption"].upper())
    y -= btn_h + 14 * mm

    # ── Idea / Empresa cards ──
    card_w = (CONTENT_W - 0.5 * mm) / 2
    card_h = 38 * mm
    cy = y - card_h
    # Card 1 · Idea
    c.setFillColor(white)
    c.rect(MARGIN_X, cy, card_w, card_h, stroke=0, fill=1)
    c.setFillColor(ACCENT)
    c.rect(MARGIN_X, cy + card_h - 1.2 * mm, card_w, 1.2 * mm, stroke=0, fill=1)
    c.setStrokeColor(LINE); c.setLineWidth(0.4)
    c.rect(MARGIN_X, cy, card_w, card_h, stroke=1, fill=0)

    cx = MARGIN_X + 6 * mm
    cyt = cy + card_h - 8 * mm
    c.setFont(F_MONO, 6.5); c.setFillColor(ACCENT)
    c.drawString(cx, cyt, t["idea_kicker"])
    # Number 01 right-aligned
    c.setFillColor(ACCENT)
    c.setFont(F_SERIF, 16)
    nx = MARGIN_X + card_w - 5 * mm - c.stringWidth("01", F_SERIF, 16)
    c.drawString(nx, cyt, "01")
    cyt -= 6 * mm
    cyt = draw_text(c, t["idea_lead"], cx, cyt, card_w - 12 * mm, F_SERIF, 12, INK, leading=15.5)
    cyt -= 1 * mm
    draw_text(c, t["idea_caption"], cx, cyt, card_w - 12 * mm, F_SANS, 8.5, INK_SOFT, leading=12)

    # Card 2 · Empresa
    cx2_box = MARGIN_X + card_w + 0.5 * mm
    c.setFillColor(white); c.rect(cx2_box, cy, card_w, card_h, stroke=0, fill=1)
    c.setFillColor(MOSS); c.rect(cx2_box, cy + card_h - 1.2 * mm, card_w, 1.2 * mm, stroke=0, fill=1)
    c.setStrokeColor(LINE); c.setLineWidth(0.4)
    c.rect(cx2_box, cy, card_w, card_h, stroke=1, fill=0)
    cx2 = cx2_box + 6 * mm
    cyt2 = cy + card_h - 8 * mm
    c.setFont(F_MONO, 6.5); c.setFillColor(MOSS)
    c.drawString(cx2, cyt2, t["comp_kicker"])
    c.setFillColor(MOSS); c.setFont(F_SERIF, 16)
    nx2 = cx2_box + card_w - 5 * mm - c.stringWidth("02", F_SERIF, 16)
    c.drawString(nx2, cyt2, "02")
    cyt2 -= 6 * mm
    cyt2 = draw_text(c, t["comp_lead"], cx2, cyt2, card_w - 12 * mm, F_SERIF, 12, INK, leading=15.5)
    cyt2 -= 1 * mm
    draw_text(c, t["comp_caption"], cx2, cyt2, card_w - 12 * mm, F_SANS, 8.5, INK_SOFT, leading=12)
    ...
    y -= 7 * mm
    # Big serif italic
    quote_lines = simpleSplit(t["promise_quote"], F_SERIF, 14.5, CONTENT_W - 5 * mm)
    c.setFillColor(INK); c.setFont(F_SERIF, 14.5)
    for ln in quote_lines:
        c.drawString(MARGIN_X, y, ln); y -= 7.5 * mm
    y -= 2 * mm
    c.setFont(F_MONO, 7); c.setFillColor(INK_SOFT)
    c.drawString(MARGIN_X, y, t["promise_caption"])
    y -= 12 * mm
    ...
    for i, (big, small) in enumerate(t["stats"]):
        ax = MARGIN_X + i * cell_w
        col = ACCENT if i in (1, 3) else INK
        c.setFillColor(col); c.setFont(F_SERIF_BOLD, 32)
        c.drawString(ax, y, big)
        c.setFont(F_MONO, 6); c.setFillColor(INK_FAINT)
        c.drawString(ax, y - 5 * mm, small)
        if i < 3:
            c.setStrokeColor(LINE); c.setLineWidth(0.4)
            c.line(ax + cell_w - 4 * mm, y - 8 * mm, ax + cell_w - 4 * mm, y + 5 * mm)
    y -= 9 * mm
    c.setStrokeColor(INK); c.setLineWidth(1)
    c.line(MARGIN_X, y, PAGE_W - MARGIN_X, y)
...
# ============================================================
# Page 2 · Casavera (sin solapamiento de número editorial)
# ============================================================
def page2(c, t):
    header_band(c, 2, 4)
    is_es = t["p2_kicker"].startswith("PROYECTO")

    # Layout: COL_NUM (50mm) + GAP (8mm) + COL_BODY (rest)
    COL_NUM_W   = 50 * mm
    GAP         = 8  * mm
    COL_BODY_W  = CONTENT_W - COL_NUM_W - GAP
    BODY_X      = MARGIN_X + COL_NUM_W + GAP

    # Section label across the top
    y = CONTENT_TOP - 6 * mm
    c.setFont(F_MONO, 7); c.setFillColor(INK_FAINT)
    c.drawString(MARGIN_X, y, t["p2_section_label"])
    y -= 18 * mm

    # Editorial number 02 in left column (HUGE but bounded)
    c.setFillColor(ACCENT); c.setFont(F_SERIF_BOLD, 84)
    c.drawString(MARGIN_X, y - 60 * mm, "02")

    # Right column · title
    title_y = y
    c.setFont(F_MONO, 6.5); c.setFillColor(ACCENT)
    c.drawString(BODY_X, title_y, t["p2_kicker"])
    title_y -= 11 * mm
    c.setFont(F_SERIF_BOLD, 36); c.setFillColor(INK)
    c.drawString(BODY_X, title_y, t["p2_h1"])
    title_y -= 7.5 * mm
    c.setFont(F_SERIF, 12.5); c.setFillColor(INK_SOFT)
    c.drawString(BODY_X, title_y, t["p2_h1_sub"])
    title_y -= 12 * mm
    # accent hairline below
    c.setStrokeColor(ACCENT); c.setLineWidth(1)
    c.line(BODY_X, title_y, BODY_X + 22 * mm, title_y)

    # Now drop below to full-width section: Needed / Delivered (2 cols)
    y = CONTENT_TOP - 80 * mm
    ...
    yy = y - 6 * mm
    yy_l = draw_text(c, t["p2_block1_body"], MARGIN_X, yy, col_w, F_SERIF, 11, INK, leading=15.5)
    # right
    rx = MARGIN_X + col_w + 8 * mm
    c.setFont(F_MONO, 6.5); c.setFillColor(ACCENT)
    c.drawString(rx, y, t["p2_block2_kicker"])
    yy = y - 6 * mm
    yy_r = draw_text(c, t["p2_block2_body"], rx, yy, col_w, F_SERIF, 11, INK, leading=15.5)

    y = min(yy_l, yy_r) - 14 * mm

    # Pull quote centered
    c.setFont(F_SERIF, 16); c.setFillColor(ACCENT)
    pq = t["p2_pull_quote"]
    pqw = c.stringWidth(pq, F_SERIF, 16)
    ...
    draw_button(c, t["p2_link1_label"], "https://resilient-arithmetic-182add.netlify.app/",
                ...
                fill=INK, text=PAPER, font_size=10.5, shadow=True, shadow_color=ACCENT)
    # Right button
    c.setFont(F_MONO, 6.5); c.setFillColor(ACCENT)
    ...
    # Layers tag
    c.setFont(F_MONO, 6.5); c.setFillColor(INK_FAINT)
    c.drawString(MARGIN_X, y, t["p2_layers_kicker"] + " · " + t["p2_layers"])
    y -= 10 * mm
    ...
    for i, (oname, obody) in enumerate(others):
        ...
        draw_text(c, obody, ox, y - 5 * mm, o_col_w, F_SANS, 9, INK_SOFT, leading=12.5, max_lines=4)
...
def page3(c, t):
    ...
    y = CONTENT_TOP - 6 * mm
    c.setFont(F_MONO, 7); c.setFillColor(INK_FAINT)
    c.drawString(MARGIN_X, y, t["p3_section_label"])
    y -= 18 * mm

    c.setFillColor(ACCENT); c.setFont(F_SERIF_BOLD, 84)
    c.drawString(MARGIN_X, y - 60 * mm, "03")

    title_y = y
    c.setFont(F_MONO, 6.5); c.setFillColor(ACCENT)
    c.drawString(BODY_X, title_y, t["p3_kicker"])
    title_y -= 11 * mm
    c.setFont(F_SERIF_BOLD, 32); c.setFillColor(INK)
    c.drawString(BODY_X, title_y, t["p3_h1"])
    title_y -= 8 * mm
    c.setFont(F_SERIF, 12); c.setFillColor(INK_SOFT)
    # subtitle wrap
    for ln in simpleSplit(t["p3_h1_sub"], F_SERIF, 12, CONTENT_W - COL_NUM_W - GAP):
        c.drawString(BODY_X, title_y, ln); title_y -= 6 * mm
    title_y -= 4 * mm
    c.setStrokeColor(ACCENT); c.setLineWidth(1)
    c.line(BODY_X, title_y, BODY_X + 22 * mm, title_y)

    # Drop below to intro + 2x2 grid
    y = CONTENT_TOP - 84 * mm
    ...
    for i, (name, sector, blurb, url) in enumerate(t["p3_cards"]):
        ...
        # Name (bold serif)
        c.setFont(F_SERIF_BOLD, 16); c.setFillColor(INK)
...
def page4(c, t):
    ...
    y = CONTENT_TOP - 6 * mm
    c.setFont(F_MONO, 7); c.setFillColor(INK_FAINT)
    c.drawString(MARGIN_X, y, t["p4_section_label"])
    y -= 18 * mm

    c.setFillColor(ACCENT); c.setFont(F_SERIF_BOLD, 84)
    c.drawString(MARGIN_X, y - 60 * mm, "04")

    title_y = y
    c.setFont(F_MONO, 6.5); c.setFillColor(ACCENT)
    c.drawString(BODY_X, title_y, t["p4_kicker"])
    title_y -= 11 * mm
    c.setFont(F_SERIF_BOLD, 30); c.setFillColor(INK)
    c.drawString(BODY_X, title_y, t["p4_h1"])
    title_y -= 8 * mm
    c.setFont(F_SERIF, 12); c.setFillColor(INK_SOFT)
    for ln in simpleSplit(t["p4_h1_sub"], F_SERIF, 12, CONTENT_W - COL_NUM_W - GAP):
        c.drawString(BODY_X, title_y, ln); title_y -= 6 * mm
    ...
    for num, name, when, price in t["capsules"]:
        ...
        if is_pack:
            c.setFillColor(ACCENT_SOFT)
            c.rect(MARGIN_X, y - row_h + 0.8 * mm, CONTENT_W, row_h - 0.8 * mm, stroke=0, fill=1)
        c.setFont(F_MONO, 9); c.setFillColor(ACCENT_DEEP if is_pack else ACCENT)
        c.drawString(MARGIN_X + 2 * mm, y - 7 * mm, num)
        c.setFont(F_SERIF_BOLD, 11.5); c.setFillColor(INK)
    ...
    draw_text(c, t["trick_body"], MARGIN_X + 8 * mm, y - 11 * mm,
              CONTENT_W - 16 * mm, F_SERIF, 10.5, INK, leading=14.5)
    y -= th + 12 * mm

    # Founder + final black CTA band
    c.setFont(F_MONO, 6.5); c.setFillColor(INK_FAINT)
    c.drawString(MARGIN_X, y, t["founder_kicker"])
    c.setFont(F_SERIF_BOLD, 20); c.setFillColor(INK)
    c.drawString(MARGIN_X, y - 8 * mm, t["founder_name"])
    c.setFont(F_SANS, 9); c.setFillColor(INK_SOFT)
    c.drawString(MARGIN_X, y - 13 * mm, t["founder_sub"])
    y -= 22 * mm
    ...
    c.drawString(MARGIN_X + 8 * mm, y - 18 * mm, t["cta_h2"])
    c.setFont(F_SANS, 9); c.setFillColor(PAPER_DEEP)
    c.drawString(MARGIN_X + 8 * mm, y - 23 * mm, t["cta_caption"])
    btn_w = 56 * mm; btn_h = 10 * mm
    draw_button(c, t["cta_btn"] + " · 30 min", CAL_URL,
